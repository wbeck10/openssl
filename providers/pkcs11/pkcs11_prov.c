...
int p11_prov_refcount;

typedef struct pkcs11_lib_handle {
    DSO *lib_handle;
    CK_OPENCRYPTOKI_FUNCTION_LIST *lib_functions;
}pkcs11_lib_handle;

static pkcs11_lib_handle *p11_lib_handle = NULL;

CK_OPENCRYPTOKI_FUNCTION_LIST *pkcs11_get_lib_functions()
{
    if (p11_lib_handle)
        return p11_lib_handle->lib_functions;
    return NULL;
}
...
static const OSSL_PARAM pkcs11_get_param_types[] = {
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_PKCS11_SLOT_INFO_VALUE,
                                                OSSL_PARAM_UTF8_PTR, NULL, 0),
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_PKCS11_SLOT_INFO_ID,
...
static const OSSL_PARAM pkcs11_set_param_types[] = {
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_PKCS11_TOKEN,   OSSL_PARAM_UTF8_PTR,     NULL, 0),
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_PKCS11_ENABLE_DIGESTS,
                                                    OSSL_PARAM_OCTET_STRING, NULL, 0),
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_PKCS11_SLOTIDX_INFO,
                                                    OSSL_PARAM_INTEGER,      NULL, 0),
    OSSL_PARAM_END
};
...
int pkcs11_generate_slots_data(PKCS11_CTX *ctx);
...
static int pkcs11_get_params(void *provctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL && !OSSL_PARAM_set_int(p, ossl_prov_is_running()))
        return 0;
    p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_PKCS11_SLOT_INFO_ID);
    if (p != NULL) {
       PKCS11_SLOT *slot = NULL;
       if (ctx->sel_slotidx_info < 0)
           return 0;
       if ((int)ctx->sel_slotidx_info >= OPENSSL_sk_num(ctx->slots))
           return 0;
       slot = (PKCS11_SLOT*)OPENSSL_sk_value(ctx->slots, ctx->sel_slotidx_info);
       if (slot == NULL)
           return 0;
       if (!OSSL_PARAM_set_int(p, slot->slotid))
           return 0;
   }
    p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_PKCS11_SLOT_INFO_VALUE);
    if (p != NULL) {
        PKCS11_SLOT *slot = NULL;
        if (ctx->sel_slotidx_info < 0)
            return 0;
        ...
        if (slot == NULL)
            return 0;
        if (!OSSL_PARAM_set_utf8_ptr(p, (const char*)slot->info.label))
            return 0;
    }
...
static int pkcs11_set_params(void *provctx, const OSSL_PARAM params[])
{
    ...
    p = OSSL_PARAM_locate((OSSL_PARAM *)params, OSSL_PROV_PARAM_PKCS11_SLOTIDX_INFO);
    if (p != NULL) {
        if (!OSSL_PARAM_get_int(p, &ival))
            return 0;
        else {
            ctx->sel_slotidx_info = ival;
        }
    }

    p = OSSL_PARAM_locate((OSSL_PARAM *)params, OSSL_PROV_PARAM_PKCS11_ENABLE_DIGESTS);
    if (p != NULL) {
        if (!OSSL_PARAM_get_int(p, &ival))
            return 0;
        else
            ctx->enable_digest = ival;
    }
    ...
    if (p != NULL) {
        if (!OSSL_PARAM_get_utf8_ptr(p, &strval))
            ...
            if (ctx->sel_token)
                OPENSSL_free(ctx->sel_token);
            ctx->sel_token = NULL;
            if (strlen(strval) > 0) {
                ctx->sel_token = OPENSSL_zalloc(strlen(strval) + 1);
                if (ctx->sel_token == NULL) {
                    SET_PKCS11_PROV_ERR(ctx, ERR_PKCS11_MEM_ALLOC_FAILED);
                    return 0;
                }
                strncpy((char *)ctx->sel_token, strval, strlen(strval));
            }
            gen_dispatch = 1;
        }
    }
    ...
    if (p != NULL) {
        if (!OSSL_PARAM_get_octet_string_ptr(p, (const void **)&ustrval, &str_len)) {
            return 0;
        } else {
            if (ctx->userpin != NULL)
    ...
    /* This parameter needs to be handled after slot or token is set */
                                          ...
                                          int *no_cache)
{
    ...
    *no_cache = 1;
    ...
    if (slot != NULL) {
        switch (operation_id) {
         /* Using digest creates issues after change cipher spec */
        case OSSL_OP_DIGEST:
            return (ctx->enable_digest ? slot->digest.algolist : NULL);
        case OSSL_OP_ASYM_CIPHER:
            return slot->asym_cipher.algolist;
...
int pkcs11_load_module(PKCS11_CTX *ctx, const char *libname)
{
    ...
    if (p11_lib_handle == NULL && libname != NULL && strlen(libname) > 0) {
        p11_lib_handle = OPENSSL_zalloc(sizeof(pkcs11_lib_handle));

        p11_lib_handle->lib_handle = DSO_load(NULL, libname, NULL, 0);
        if (p11_lib_handle->lib_handle == NULL)
            goto end;

        fun_ptr = (get_func_list *)DSO_bind_func(p11_lib_handle->lib_handle, "C_GetFunctionList");
        if (fun_ptr == NULL)
            goto end;

        rv = fun_ptr((CK_FUNCTION_LIST **)&p11_lib_handle->lib_functions);
        if (rv != CKR_OK) {
        ...
        rv = p11_lib_handle->lib_functions->C_Initialize(&cinit_args);
...
int pkcs11_generate_slots_data(PKCS11_CTX *ctx)
{
    ...
    rv = pkcs11_get_lib_functions()->C_GetSlotList(TRUE, NULL_PTR, &slot_count);
    if (rv != CKR_OK) {
        SET_PKCS11_PROV_ERR(ctx, rv);
        goto end;
    }
    ...
    rv = pkcs11_get_lib_functions()->C_GetSlotList(TRUE, slot_list, &slot_count);
    if (rv != CKR_OK) {
    ...
    for (i = 0; i < slot_count; i++) {
        ...
        rv = pkcs11_get_lib_functions()->C_GetTokenInfo(slot, &pkcs11_slot->info);
        ...
        rv = pkcs11_get_lib_functions()->C_GetMechanismList(slot, NULL, &mechcount);
        if (rv != CKR_OK) {
            SET_PKCS11_PROV_ERR(ctx, rv);
            goto end;
        }
        ...
        rv = pkcs11_get_lib_functions()->C_GetMechanismList(slot, mechlist, &mechcount);
        if (rv != CKR_OK) {
            SET_PKCS11_PROV_ERR(ctx, rv);
            goto end;
        }
        ...
        pkcs11_slot->asym_cipher.items = OPENSSL_sk_new_null();
        if (pkcs11_slot->asym_cipher.items == NULL) {
            SET_PKCS11_PROV_ERR(ctx, ERR_PKCS11_MEM_ALLOC_FAILED);
            goto end;
        }
        ...
        for (i = 0; i < mechcount; i++) {
            rv = pkcs11_get_lib_functions()->C_GetMechanismInfo(slot,
                                                                mechlist[i],
                                                                &mechinfo[i]);
            if (rv != CKR_OK) {
                SET_PKCS11_PROV_ERR(ctx, rv);
            ...
            if ((mechinfo[i].flags & CKF_ENCRYPT) &&
                    (mechinfo[i].flags & CKF_DECRYPT)) {
                PKCS11_TYPE_DATA_ITEM *item = OPENSSL_zalloc(sizeof(*item));
                if (item) {
                    item->info = mechinfo[i];
                    item->type = mechlist[i];
                    OPENSSL_sk_push(pkcs11_slot->asym_cipher.items, item);
                }
            }
        }
        PKCS11_TYPE_DATA_ITEM *item = OPENSSL_zalloc(sizeof(*item));
    ...
    if (ctx->slots != NULL) {
        for (i = 0; i < OPENSSL_sk_num(ctx->slots); i++) {
            ...
            if (slot != NULL) {
                ...
                if (slot->asym_cipher.items != NULL) {
                    for (ii = 0; ii < OPENSSL_sk_num(slot->asym_cipher.items); ii++) {
                        item = (PKCS11_TYPE_DATA_ITEM *)OPENSSL_sk_value(slot->asym_cipher.items, ii);
                        OPENSSL_free(item);
                    }
                    OPENSSL_sk_free(slot->asym_cipher.items);
                }
                OPENSSL_free(slot->asym_cipher.algolist);
...
void pkcs11_unload_module(PKCS11_CTX *ctx)
{
    if (p11_lib_handle != NULL)
    {
        if (p11_prov_refcount <= 0) {
            if (p11_lib_handle->lib_functions != NULL) {
                p11_lib_handle->lib_functions->C_Finalize(NULL);
                p11_lib_handle->lib_functions = NULL;
            }
            DSO_free(p11_lib_handle->lib_handle);
            p11_lib_handle->lib_handle = NULL;
            OPENSSL_free(p11_lib_handle->lib_functions);
            OPENSSL_free(p11_lib_handle);
            p11_lib_handle = NULL;
            pkcs11_keygmgmt_free_lock();
        }
    }
    if (ctx->sel_token)
        OPENSSL_free(ctx->sel_token);
...
int pkcs11_generate_dispatch_tables(PKCS11_CTX *ctx)
{
    ...
    id = ctx->search_str;
    if (ctx->slots == NULL)
        pkcs11_generate_slots_data(ctx);
    ...
    if (slot != NULL) {
        ...
        if (slot->asym_cipher.algolist == NULL)
            slot->asym_cipher.algolist = pkcs11_asym_cipher_get_algo_tbl(slot->asym_cipher.items, id);
        if (slot->store.algolist == NULL)
            slot->store.algolist = pkcs11_store_get_algo_tbl(id);
    }
    return 1;
}

...
int pkcs11_digest_required_length(PKCS11_DIGEST_CTX *ctx, size_t *len);
...
static int pkcs11_digest_default_init(void *dctx, const OSSL_PARAM params[],
                                      unsigned long type, int nid, int digest_size);
...
{                                                                                       \
    return pkcs11_digest_default_init(dctx, params, ckmid, nid, dgstsize);              \
}                                                                                       \
...
const PKCS11_DIGEST_MAP pkcs11_digest_map[] = {
    ...
    {CKM_SHA_1,      PROV_NAMES_SHA1,         NID_sha1,       PKCS11_DIGEST_DB_TBL(sha1)},
    {CKM_SHA224,     PROV_NAMES_SHA2_224,     NID_sha224,     PKCS11_DIGEST_DB_TBL(sha224)},
    {CKM_SHA256,     PROV_NAMES_SHA2_256,     NID_sha256,     PKCS11_DIGEST_DB_TBL(sha256)},
    {CKM_SHA384,     PROV_NAMES_SHA2_384,     NID_sha384,     PKCS11_DIGEST_DB_TBL(sha384)},
    {CKM_SHA512,     PROV_NAMES_SHA2_512,     NID_sha512,     PKCS11_DIGEST_DB_TBL(sha512)},
    {CKM_SHA512_224, PROV_NAMES_SHA2_512_224, NID_sha512_224, PKCS11_DIGEST_DB_TBL(sha512_224)},
    {CKM_SHA512_256, PROV_NAMES_SHA2_512_256, NID_sha512_256, PKCS11_DIGEST_DB_TBL(sha512_256)},
    {0, NULL, 0, NULL}
};
                                      ...
                                      unsigned long type, int nid, int digest_size)
{
    ...
    ctx->mechdata = pkcs11_digest_get_mech_data(ctx->pkcs11_ctx, type);
    ctx->is_init = 1;
...
static int pkcs11_digest_update(void *dctx, const unsigned char *in, size_t inl)
{
    ...
    if (ctx->is_init) {
        ...
        rv = pkcs11_get_lib_functions()->C_DigestInit(ctx->session,
                                                      &mech);
        if (rv != CKR_OK) {
    ...
    rv = pkcs11_get_lib_functions()->C_DigestUpdate(ctx->session,
                                                    (CK_BYTE *)in, ckinl);
    if (rv != CKR_OK) {
        SET_PKCS11_PROV_ERR(ctx->pkcs11_ctx, rv);
        goto end;
                               ...
                               size_t outsz)
{
    ...
    if (out == NULL) {
        ret = pkcs11_digest_required_length(ctx, outl);
        goto end;
    }

    rv = pkcs11_get_lib_functions()->C_DigestFinal(ctx->session,
                                                   (CK_BYTE *)out, &ckoutl);
    if (rv != CKR_OK) {
        SET_PKCS11_PROV_ERR(ctx->pkcs11_ctx, rv);
        goto end;
...
static void pkcs11_digest_freectx(void *dctx)
{
    ...
    if (digestctx != NULL) {
        ...
        if (ctx != NULL) {
            pkcs11_close_session(ctx, &digestctx->session);
        }
        OPENSSL_free(digestctx);
    }
}

static void *pkcs11_digest_dupctx(void *ctx)
{
    PKCS11_DIGEST_CTX *in = (PKCS11_DIGEST_CTX *)ctx;
    PKCS11_DIGEST_CTX *ret = NULL;
    int ok = 0;
    ...
    ret = OPENSSL_zalloc(sizeof(PKCS11_DIGEST_CTX));
    if (ret == NULL)
        goto end;

    ret->pkcs11_ctx = in->pkcs11_ctx;
    if (in->mechdata) {
        ret->mechdata = OPENSSL_malloc(sizeof(PKCS11_TYPE_DATA_ITEM));
        ret->mechdata->info = in->mechdata->info;
        ret->mechdata->type = in->mechdata->type;
    }
    ret->nid = in->nid;
    ret->digest_size = in->digest_size;
    ret->is_init = 1;

    if (in->session != 0)
        ok = pkcs11_open_session(ret->pkcs11_ctx, &ret->session);
    if (!ok)
        goto end;
    ...
    if (!ok) {
        if (ret) {
            pkcs11_close_session(ret->pkcs11_ctx, &ret->session);
            if (ret->mechdata)
                OPENSSL_free(ret->mechdata);
            OPENSSL_free(ret);
        }
        ret = NULL;
    }
...
int pkcs11_find_mdnid_by_name(const char* name)
{
    int ret = NID_undef;
    const PKCS11_DIGEST_MAP *p = pkcs11_digest_map;
    char *pos = NULL;
    int ipos = 0;
    ...
    for (; p->mechanics != 0; p++) {
        mapnamelen = strlen(p->SN);
        while((pos = strstr(p->SN, name)) != NULL) {
            ipos = pos - p->SN;
            if ((ipos + namelen) < mapnamelen) {
                char *pp = (pos + namelen);
                if ((*pp) == ':' || (*pp) == '\0') {
                    ret = p->nid;
                    goto end;
